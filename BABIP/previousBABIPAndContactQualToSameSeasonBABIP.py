...
X = contact_data.drop("babip", axis = 1)
y = contact_data.babip
...
plt.title('Actual vs Predicted BABIP (maybe better)')
#r2 = model.score(X_test, y_test) #calculate the r^2
r2 = r2_score(y_test, y_pred)
print(f"Multi Lin Reg v2, R^2 = {r2}")
plt.text(.3, .26, f"r^2: {r2}") # show the r^2
plt.text(.3, .265, f"Lin Reg v2, Num Samples: {X.shape[0]}")
plt.show()
